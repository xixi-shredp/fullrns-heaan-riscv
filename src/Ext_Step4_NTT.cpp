...
static inline void ext_rowNTTWithBar(uint64_t *a, long N, long logN, uint64_t q,
...
static inline void ext_rowNTTWithMont(uint64_t *a, long N, long logN,
...
static inline void ext_row_mul_withBar(long rowIdx, long rowSz, long logRowSz,
                                       uint64_t *src, uint64_t *wmatrix_pows,
...
static inline void ext_row_mul_withMont(long rowIdx, long rowSz, long logRowSz,
                                        uint64_t *src,
                                        uint64_t *wmatrix_scalepows, long q,
                                        long qInv) {
  for (int j = 0; j < rowSz; ++j) {
    uint64_t idx = (rowIdx << logRowSz) + j;
    uint64_t T = src[idx];
...
#define ROW_MUL_Montgomeny                                                     \
  ext_row_mul_withMont(i, N2, logN2, a, wmatrix_scalepows[index], q, qInv);

#define ROW_NTT_Barrett(a, s, N)                                               \
  {                                                                            \
    uint64_t *__a1 = a + (i << log##N);                                        \
    ext_rowNTTWithBar(__a1, N, log##N, q, s4ntt_##s##_qRootPows[index],        \
