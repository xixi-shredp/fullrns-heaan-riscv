...
#define ROW_MUL_Barrett(mod)                                 \
  row_mul_withBar(i, N2, logN2, a, mod##Wmatrix_pows[index], \
                  s4ntt_##mod##WmatrixBarPres[index], mod, mod##Inv);

#define ROW_MUL_Montgomeny(mod) \
  row_mul_withMont(i, N2, logN2, a, mod##Wmatrix_scalepows[index], mod, mod##Inv);

#define ROW_NTT_Barrett(a, s, N, mod)                                       \
  {                                                                         \
    uint64_t *__a1 = a + (i << log##N);                                     \
    rowNTTWithBar(__a1, N, log##N, mod, s4ntt_##s##_##mod##RootPows[index], \
...
#define ROW_NTT_Montgomeny(a, s, N, mod)                     \
  {                                                          \
    uint64_t *__a1 = a + (i << log##N);                      \
    rowNTTWithMont(__a1, N, log##N, mod, mod##Inv,           \
                   s4ntt_##s##_##mod##RootScalePows[index]); \
  }

STEP4NTT_TEMPLATE_IMPLEMENT(step4_qiNTTAndEqual_withBar, ROW_MUL_Barrett,
                            ROW_NTT_Barrett, NO_SET_MOD, q);

STEP4NTT_TEMPLATE_IMPLEMENT(step4_qiNTTAndEqual_withMont, ROW_MUL_Montgomeny,
                            ROW_NTT_Montgomeny, NO_SET_MOD, q);

STEP4NTT_TEMPLATE_IMPLEMENT(step4_piNTTAndEqual_withBar, ROW_MUL_Barrett,
                            ROW_NTT_Barrett, NO_SET_MOD, p);

STEP4NTT_TEMPLATE_IMPLEMENT(step4_piNTTAndEqual_withMont, ROW_MUL_Montgomeny,
                            ROW_NTT_Montgomeny, NO_SET_MOD, p);
